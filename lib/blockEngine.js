...
const createMovieBlock = (movies,
    ...
    let selectedBuffer = bufferEng
        .streamBufferGenerator(selectedMovie.DurationLimit - selectedMovie.Duration,
    ...
    items.push(selectedMovie.MoviePath);
...
const createShowBlock = (duration,
    shows,
    commercials,
    promos,
    shorts,
    ...
    if (currentDuration > 1800) {
        ...
        //Get or Create progression in in progression file
        //If progression is undefined override progression object with a random number 
        //!!! Important Impliment progression save mechanic to make sure temporary progression does not save after stream end
        ...
        let nonOverDurationShows = shows.filter(item => item.OverDuration === false);
...
exports.createProceduralBlock = (options, duration, shows, movies, commercials, promos, shorts, music) => {
    ...
    while (tagComboClipsDuration > currentTagComboClipsDuration) {
        ...
        if (coin < 0.5
            ...
            let showBlock = createMovieBlock(showsWithinDuration,
                commercials,
                promos,
                shorts,
                music,
