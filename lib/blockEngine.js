...
const loadUniqueMoviesUnderDuration = (duration, movieList, previousMovies) => {
    let uniqueMovies = [];
...
const hasUniqueClipsUnderDurationAvailable = (duration, movieList, previousMovies) => {
    let targetClips = loadUniqueMoviesUnderDuration(duration, movieList, previousMovies);
    return targetClips.length > 0;
}

exports.createProceduralBlock = (options, duration, shows, movies, commercials, promos, shorts, music) => {
    ...
    while (tagComboClipsDuration > currentTagComboClipsDuration) {
        ...
        if (coin < 0.5
            && hasUniqueClipsUnderDurationAvailable(tagComboClipsDuration - currentTagComboClipsDuration,
                selectedComboMedia.movies,
                createdBlock.Movies)) {
            let movieBlock = [];
            let selectedMovie = selectedComboMedia.movies[Math.floor(Math.random() * indexes.length) | 0];

            movieBlock.push(selectedMovie.MoviePath);
            ...
            let selectedBuffer = bufferEng
                .streamBufferGenerator(sortedShow.DurationLimit - currentShowBlockDuration,
        ...
        availableMedia[Math.floor(Math.random() * indexes.length) | 0];
    }
