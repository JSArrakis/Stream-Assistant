...
const utilities = require("./utilitiesdeprecated");
...
const loadTagComboMedia = (options, showList, movieList) => {
    ...
    //TODO: Test this
    options.tagsAND.forEach(combo => combos.push(_.split(combo, ",")));

    //TODO: Test this
    if (options.tagsOR === undefined) {
        let tags = [];
        combos.forEach(combo => tags.push(...combo));
        options.tagsOR = _.uniq(tags);
    }
...
const loadTagMedia = (options, showList, movieList) => {
    ...
    if (options.tagsOR === undefined) {
        let tags = [];
        showList.forEach(show => tags.push(...show.Tags));
        movieList.forEach(movie => tags.push(...movie.Tags));
        options.tagsOR = _.uniq(tags).sort();
    }
...
const getShow = (
    ...
    showsWithinDuration.forEach(item => {
        if (item.LoadTitle.startsWith("temp")) {
            let episodeCount = previousTagEpisodes.filter(x => x.LoadTitle === item.LoadTitle);
            if (episodeCount.length < 1) {
                showList.push(item);
                previousTagEpisodes.push({
                    LoadTitle: item.LoadTitle,
                    NumberOfEpisodes: 0
                })
            } else if (previousTagEpisodes.filter(x => x.LoadTitle === item.LoadTitle)[0].NumberOfEpisodes < item.EpisodeCount) {
                showList.push(item);
            }
    ...
    let tempShowsWithinDuration = showsWithinDuration.filter(show => show.LoadTitle.startsWith("temp"));
    let tempShowList = showList.filter(show => show.LoadTitle.startsWith("temp"));
    if (tempShowsWithinDuration.length > 0 && tempShowList.length < 1) {
        previousTagEpisodes = [];
        tempShowsWithinDuration.forEach(item => {
            showList.push(item);
            previousTagEpisodes.push({
                LoadTitle: item.LoadTitle,
                NumberOfEpisodes: 0
    ...
    if (duration > 1800) {
        show = showList[Math.floor(Math.random() * showList.length) | 0];
        ...
        if (show.DurationLimit > 1800) {
            ...
            let selectedEpisode = show.Episodes[currentEpisode - 1];
            previousTagEpisodes.filter(x => x.LoadTitle === show.LoadTitle).forEach(y => y.NumberOfEpisodes++);
            incrementProgression(progression, "Procedural", show);
        ...
        let halfHourShows = showList.filter(item => item.DurationLimit === 1800);
...
const selectMedia = (options,
    ...
    while (mediaDuration > currentDuration) {
        ...
        if (currentTimePointDay !== currentDay) {
            previousMovies = [];
            currentDay = currentTimePointDay;
        }
        ...
        if (hasUniqueMoviesWithinDurationAvailable(duration,
            ...
            currentTimePoint = currentTimePoint + movieBlock.Movie.DurationLimit;
            ...
            if (coin < 0.5
                ...
                if (showBlock.Show.LoadTitle)

                    shows.push(showBlock);
                currentDuration = currentDuration + (showBlock.Show.DurationLimit * showBlock.Episodes.length);
                currentTimePoint = currentTimePoint + (showBlock.Show.DurationLimit * showBlock.Episodes.length);
...
exports.createScheduledMovieBlocks = (options,
    movieList) => {
    ...
    if (options.movies && options.movies.length > 0) {
        let blocks = [];
        options.movies.forEach(item => {
            if (item.includes("::")) {
                blocks.push(item);
            }
        });
        scheduledMovies.push(...parseBlocks(blocks))
    }

    scheduledMovies.forEach(item => {
        let movie = movieList.filter(m => m.LoadTitle === item.Title)[0];
        if (movie === undefined) {
            throw item.Title + " does not exist as a load title in the movie list."
        }
        ...
        createdBlock.Items.push(movie.MoviePath);
        createdBlock.Duration = movie.Duration;

        let selectedBuffer = bufferEng
            .createProceduralBlockBuffer(createdBlock.DurationLimit - movie.Duration,
                blockCommercials,
                blockPromos,
                blockShorts,
                blockMusic,
        ...
        createdBlock.Duration = createdBlock.Duration + selectedBuffer.Duration;

        createdBlock.Programming.push({
            mediaName: movie.Title,
            mediaStartTime: item.Time,
            createdBy: "Scheduled Movie"
        })
