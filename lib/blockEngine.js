...
const createShowBlock = (
    ...
    if (currentDuration > 1800) {
        ...
        let selectedShow = showsWithinDuration[Math.floor(Math.random() * showsWithinDuration.length) | 0];

        createProgression(options, progression, "Procedural", selectedShow);
        ...
        if (selectedShow.DurationLimit > 1800) {
            ...
            incrementProgression(progression, "Procedural", selectedShow);
        ...
        let nonOverDurationShows = showsWithinDuration.filter(item => item.OverDuration === false && item.DurationLimit === 1800);
...
const selectMedia = (media,
    ...
    let previousCommercials = initialCommercials;
    let previousPromos = initialPromos;
    let previousShorts = initialShorts;
    let previousMusic = initialMusic;
    ...
    return {
        "MovieBlocks": createdBlock.MovieBlocks,
        "ShowBlocks": createdBlock.ShowBlocks
    }
}

exports.createProceduralBlock = (options, duration, shows, movies, commercials, promos, shorts, music, stagedProgression, masterProgression) => {

    if (!options.progression) {
        if (!stagedProgression.some(progItem => progItem.Title === "Procedural")) {
    ...
    } else if (!stagedProgression.some(progItem => progItem.Title === "Procedural")) {
        stagedProgression.push({
            "Title": "Procedural",
            "Type": "Procedural",
            "Shows": new []
    ...
    let tagComboMedia = selectMedia(selectedComboMedia,
        tagComboClipsDuration,
        commercials,
        promos,
        shorts,
    ...
    let tagMedia = selectMedia(selectedTagMedia,
        duration - tagComboClipsDuration,
        commercials,
        promos,
        shorts,
        music,
    ...
    movieBlocks.push(...tagComboMedia.MovieBlocks);
    movieBlocks.push(...tagMedia.MovieBlocks);
    showBlocks.push(...tagComboMedia.ShowBlocks);
    showBlocks.push(...tagMedia.ShowBlocks);
        ...
        sortedBlockShows.forEach(sortedShow => {
            ...
            if (!skipShow) {
                ...
                if (selectedShow !== undefined) {
                    ...
                    //first episode.
                    createProgression(options, stagedProgression, blockPaths, selectedShow);
                    ...
                    let shuffledBuffer = bufferEng
                        .collectionBufferShuffle(selectedBuffer.selectedCommercials,
                            selectedBuffer.selectedPromos,
