...
exports.createProgressionFile = async (destinationDirectory, progression) => {
    return await fs.promises.writeFile(destinationDirectory + 'progression.json', JSON.stringify(progression), (err) => {
        if (err) throw err;
...
exports.createStreamFile = async (destinationDirectory, playlistFileName, stream) => {
    const newline = "\r\n"
    return await fs.promises.writeFile(destinationDirectory + playlistFileName + '.m3u', stream.join(newline), (err) => {
        if (err) throw err;
        console.log('Stream file is created successfully.');
    });
}