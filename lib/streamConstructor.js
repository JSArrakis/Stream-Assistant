...
exports.constructStream = (config, destinationDirectory, options) => {
    ...
    */
    ...
    let forcedProgressionBlocks = [];
    let regularProgressionBlocks = [];
    blockList.forEach(b => {
        if (b.Type === "Collection")
        {
            forcedProgressionBlocks.push(b);
        } else {
            regularProgressionBlocks.push(b);
        }
    })
    
    forcedProgressionBlocks.forEach(b => progression.filter(pitem => pitem.Title === b.Title)[0]
        .Shows.forEach(sitem => sitem.Episode++));
    
    if (options.progression)
    {
        regularProgressionBlocks.forEach(b => progression.filter(pitem => pitem.Title === b.Title)[0]
            .Shows.forEach(sitem => sitem.Episode++));
    }
