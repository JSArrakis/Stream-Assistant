...
exports.bufferShuffle = (commercials, promos, shorts, music) => {
    let sequence = [];
    let combo = [];
    let designatedPromo = undefined;
    ...
    if (promos.length > 0) {
        designatedPromo = promos[0];
        combo.push(...promos.slice(1));
    }
...
const loadUniqueMediaUnderDuration = (target,
    ...
    promosUnderTarget.forEach(clip => {
        if (previousPromos.indexOf(clip) === -1) {
            targetClips.push(clip);
        }
    })
 ...
 */
const loadProceduralBufferBlock = (duration, highestMediaDuration, commercials, promos, shorts, music, initialPromo) => {
    const target = duration - highestMediaDuration;
    ...
    while (hasUniqueMediaUnderDurationAvailable(target - currentDuration,
        ...
        let tempList = loadUniqueMediaUnderDuration(target - currentDuration,
            commercials,
            promos,
            shorts,
            music,
...
exports.streamBufferGenerator = (duration, commercials, promos, shorts, music, defaultPromo) => {
    ...
    */
    if (duration > 15) {
        ...
        //Find the highest duration to allow a duration at the end of the buffer to ensure the final clip
        //selected is never longer than the remainder of the buffer
        let highestClipDuration = getHighestMediaDuration(remainder,
            commercials,
            promos,
            shorts,
            music);
        ...
        //Check if any clips fit the remainder, if not, skip as it is unlikely to be a duration that matters. The 
        //offset will be corrected by the cadence engine
        let finalClips = loadUniqueMediaUnderDuration(remainder + 5,
            commercials,
            promos,
            shorts,
            music,
