...
/**
 * @param duration The duration in seconds of the buffer time needed to be created to ensure the next show starts on the half hour or hour block mark.
 * @param commercialList List of commercial clips that have both a path and duration property, where the path is the path to the commercial clip and the duration is the length in seconds of the commercial.
 * @param promoList List of promo clips relative to the stream and block that have both a path and duration property, where the path is the path to the promo clip and the duration is the length in seconds of the promo.
 */
const getHighestClipDuration = (duration, commercialList, promoList) => {
    //Combine all clips into a single temporary list for duration evaluation to ascertain highest duration available within the { duration } available
    let tempCombined = [];
...
/**
 * @param duration The duration in seconds of the buffer time needed to be created to ensure the next show starts on the half hour or hour block mark.
 * @param highestClipDuration Highest duration in seconds of clips from the commercial and promo lists that fit within the { duration }
 * @param commercialList List of commercial clips that have both a path and duration property, where the path is the path to the commercial clip and the duration is the length in seconds of the commercial.
 * @param promoList List of promo clips relative to the stream and block that have both a path and duration property, where the path is the path to the promo clip and the duration is the length in seconds of the promo.
 */
    ...
    if (fullRemainderClips.length < 1) {
        ...
        if (fullRemainderClips.length === 2) {
            if (fullRemainderClips[0].duration === fullRemainderClips[0].duration) {
                ...
                //TODO: Check to see which clip is closer to the remainder time and pick the closest using absolute value
            }

        } else if (fullRemainderClips.length === 1) {

        } else {
            //TODO: Check to see which clip is closer to the remainder time and pick the closest using absolute value. If multiple are the "closest", pick one at random from among those results
        }
    }
    return {
        commercials: selectedCommercials,
 ...
 * @param commercialList List of commercial clips that have both a path and duration property, where the path is the path to the commercial clip and the duration is the length in seconds of the commercial.
 * @param promoList List of promo clips relative to the stream and block that have both a path and duration property, where the path is the path to the promo clip and the duration is the length in seconds of the promo.
 * @param fillerShowList List of short shows (15 minutes or less) relative to the stream and block that have both a path and duration property, where the path is the path to the show and the duration is the length in seconds of the show.
 */
exports.bufferGenerator = (duration, commercialList, promoList, fillerShowList) => {
    ...
    //Check to see if Promo List has at least one item that has a duration less than or equal to { duration } as there is an inherent rule here baked in that there will always be at least 1 promo item between each episode (Promos are both Block and stream specific)
    if (promoList.some(item => item.duration <= duration)) {

        //Find the highest duration to allow a buffer at the end of the buffer, to ensure there is space to select a final clip from any duration available.
        highestClipDuration = getHighestClipDuration(duration, commercialList, promoList);

        //Select One Promo that has a duration less than or equal to the { duration } time and add it to selected Promos. There will always be one Promo per buffer if available.
        let defaultPromo = promoList.filter(item => item.duration <= duration)[Math.floor(Math.random() * tempList.length) | 0];
        selectedPromos.push(defaultPromo);
        currentDuration = currentDuration + defaultPromo.duration;

        //Select shows for buffer leaving a remainder duration to be filled byt a different process in order to ensure show cadence
        ...
        //Check if any clips fit the remainder, if not, skip as it is unlikely to be a duration that matters. The offset will be corrected by the cadence engine
        let finalClips = loadRemainderClips(remainder, commercialList, promoList, selectedCommercials, selectedPromos);
