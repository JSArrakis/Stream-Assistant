import { MediaProgression, Progression } from "../models/mediaProgression";
import { Show } from "../models/show";
import { IStreamRequest } from "../models/v1/streamRequest";
import { keyNormalizer } from "../utils/utilities";
import { getStreamArgs, getStreamType } from "./streamManager";
...
let mediaProgressionList: MediaProgression[] = [];

export function SetProgression(prog: MediaProgression[]) {
...
export function GetMediaProgression(mediaTitle: string, mediaLoadTitle: string, environment: string, type: StreamType): MediaProgression {
    ...
    if (!mediaProgression) {
        mediaProgression = new MediaProgression(mediaTitle, mediaLoadTitle, environment, type, []);
...
export function GetProgression(mediaProgression: MediaProgression, title: string): Progression {
    ...
    if (!progression) {
        progression = new Progression(title, keyNormalizer(title), 1, 0);
...
export function GetEpisodesToPlay(mediaLoadTitle: string, show: Show, progression: Progression, numberOfEpisodes: number): number[] {
    // Array to hold the episode numbers
    let episodeNumbers: number[] = [];
...
export function AddAnthologyProgression(title: string, type: string, progression: MediaProgression[], anthology: string) {
