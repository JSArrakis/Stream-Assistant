...
export async function createShortHandler(req: Request, res: Response): Promise<void> {
...
export async function bulkCreateShortHandler(req: Request, res: Response): Promise<void> {
    // Check for validation errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        res.status(400).json({ errors: errors.array() });
    ...
    if (!req.body.every((item: any) => typeof item === 'object')) {
        res.status(400).json({ message: "Request body must be an array of short objects" });
        return;
    }
    ...
    for (const shortEntry of req.body) {
        ...
        try {
            let loadTitle = shortEntry.title.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
            const short = await ShortModel.findOne({ LoadTitle: loadTitle });
            if (short) {
                // If it exists, return error
                responseErrors.push(new LoadTitleError(shortEntry.title, `Short ${shortEntry.loadTitle} already exists`));
                continue;
            }

            let transformedComm = await transformShortFromRequest(shortEntry, loadTitle);

            await ShortModel.create(transformedComm);
            createdShorts.push(transformedComm.LoadTitle);
        } catch (err) {
            responseErrors.push(new LoadTitleError(shortEntry.loadTitle, err as string));
        }
    }

    if (responseErrors.length === req.body.length) {
        res.status(400).json({ message: "Shorts Not Created", createdShorts: [], errors: responseErrors });
        return;
    }

    res.status(200).json({ message: "Shorts Created", createdShorts: createdShorts, errors: responseErrors });
    return;
}

export async function deleteShortHandler(req: Request, res: Response): Promise<void> {
    // Check for validation errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        res.status(400).json({ errors: errors.array() });
...
export async function updateShortHandler(req: Request, res: Response): Promise<void> {
    // Check for validation errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        res.status(400).json({ errors: errors.array() });
...
export async function getAllShortsHandler(req: Request, res: Response): Promise<void> {
...
export async function transformShortFromRequest(short: any, loadTitle: string): Promise<Short> {
